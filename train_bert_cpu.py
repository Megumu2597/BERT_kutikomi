...
from transformers import BertTokenizer, BertForSequenceClassification, pipeline, BertConfig, BertModel
...
demographics = pd.read_csv('/home/kwk/share/retty/u00643/data/prj_dss_genderage_fold_by_park.csv')
...
class DatasetRetriever(Dataset):
    def __init__(self, data, tokenizer, max_len, is_test=False):
        ...
        if 'review' in self.data.columns:
            self.excerpts = self.data.review.values.tolist()
        else:
            self.excerpts = self.data.text.values.tolist()
        self.targets = self.data.gender.values.tolist()
...
class RettyModel(nn.Module):
    def __init__(
        ...
        super(RettyModel, self).__init__()
        self.config = config
...
def get_optimizer_params(model):
    ...
    optimizer_parameters = [
        {'params': [p for n, p in model.bert.named_parameters() if not any(nd in n for nd in no_decay) and not any(nd in n for nd in group_all)],'weight_decay': 0.01},
        {'params': [p for n, p in model.bert.named_parameters() if not any(nd in n for nd in no_decay) and any(nd in n for nd in group1)],'weight_decay': 0.01, 'lr': learning_rate/2.6},
        {'params': [p for n, p in model.bert.named_parameters() if not any(nd in n for nd in no_decay) and any(nd in n for nd in group2)],'weight_decay': 0.01, 'lr': learning_rate},
        {'params': [p for n, p in model.bert.named_parameters() if not any(nd in n for nd in no_decay) and any(nd in n for nd in group3)],'weight_decay': 0.01, 'lr': learning_rate*2.6},
        ...
        {'params': [p for n, p in model.named_parameters() if "bert" not in n], 'lr':1e-3, "momentum" : 0.99},
    ]
    return optimizer_parameters


def make_model(model_name='cl-tohoku/bert-base-japanese-char', num_labels=1):
    tokenizer = BertTokenizer.from_pretrained(model_name)
    config = BertConfig.from_pretrained(model_name)
    config.update({'num_labels':num_labels})
    model = RettyModel(model_name, config=config)
...
class Trainer:
    def __init__(self, model, optimizer, scheduler, scalar=None, log_interval=1, evaluate_interval=1000):
        self.model = model
...
def config(fold=0):
    ...
    model, tokenizer = make_model(model_name='cl-tohoku/bert-base-japanese-char', num_labels=1)
...
with open('bert_log_list.pickle', 'wb') as handle:
